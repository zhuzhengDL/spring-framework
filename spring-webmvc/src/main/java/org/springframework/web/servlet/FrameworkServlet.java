...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	/** 默认创建的 WebApplicationContext 类型
	 ...
	 *  用于在其中查找 WebApplicationContext 的 ServletContext 属性。
	 * */
	@Nullable
	 ...
	 * 我们应该在每个请求结束时发布一个 ServletRequestHandledEvent 吗？
	 * */
	 ...
	 *  我们应该向 {@link #doService} 发送 HTTP OPTIONS 请求吗？
	 * */
	private boolean dispatchOptionsRequest = false;

	/** Should we dispatch an HTTP TRACE request to {@link #doService}?.
	 *  我们应该向 {@link #doService} 发送 HTTP TRACE 请求吗？
	 * */
	...
	/** Spring 通过 {@link ApplicationContextAware} 调用以注入当前应用程序上下文
	...
	/** {@link HttpServletBean} 的重写方法，在 bean 属性设置后调用。创建此 servlet 的 WebApplicationContext。
	 *
	...
	protected WebApplicationContext initWebApplicationContext() {
		// <1> 获得 根 WebApplicationContext 对象
	...
	protected WebApplicationContext createWebApplicationContext(@Nullable ApplicationContext parent) {
		// <a> 获得 context 的类 默认 XmlWebApplicationContext.class;
	...
	protected void doOptions(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		// 如果 dispatchOptionsRequest 为 true ，则处理该请求  默认false
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// <4> 返回当前绑定到线程的 RequestAttributes。
	...
	/** 为给定的请求构建 ServletRequestAttributes（可能还持有对响应的引用），考虑预先绑定的属性（及其类型）。
	 *
